...
const InterierTabModel = require("../../models/tabs/Interier");
...
router.delete("/interiertab/:id", async (req, res) => {
  try {
    const { id } = req.params;
...
router.get("/interiertab", async (req, res) => {
  try {
    const data = await InterierTabModel.find();
    if (!data) {
      return res.status(404).json({ message: "data not found" });
    }
...
router.get("/interiertabfront", async (req, res) => {
  try {
...
router.post("/status-update-interier/:id", async (req, res) => {
  try {
    const { id } = req.params;
...
router.get("/status-interier/:id", async (req, res) => {
  try {
    const { id } = req.params;
