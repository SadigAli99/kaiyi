...
router.post("/dealertab", uploadConfig.single("img"), async (req, res) => {
  try {
    ...
    const createData = new BecomeDealerModel({
      title: {
...
router.put("/dealertab/:id", uploadConfig.single("img"), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const updatedData = await BecomeDealerModel.findByIdAndUpdate(
      id,
      {
        $set: {
          title: {
...
router.delete("/dealertab/:id", async (req, res) => {
  try {
    const { id } = req.params;

    const data = await BecomeDealerModel.findById(id);

    if (!data) {
      return res.status(404).json({ message: "data not found." });
    }

    await BecomeDealerModel.findByIdAndDelete(id);
...
router.get("/dealertab", async (req, res) => {
  try {
    const data = await BecomeDealerModel.find();
    if (!data) {
      return res.status(404).json({ message: "data not found" });
    }
...
router.get("/dealertabfront", async (req, res) => {
  try {
    const acceptLanguage = req.headers["accept-language"];
    const preferredLanguage = acceptLanguage.split(",")[0].split(";")[0];

    const datas = await BecomeDealerModel.find({ status: "active" });
...
router.post("/status-update-dealertab/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { status } = req.body;
    const updatedData = await BecomeDealerModel.findByIdAndUpdate(id, { status }, { new: true });
...
router.get("/status-dealertab/:id", async (req, res) => {
  try {
    const { id } = req.params;

    const data = await BecomeDealerModel.findById(id, "status");
