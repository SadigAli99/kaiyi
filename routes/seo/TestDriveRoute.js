...
const SeoTestDriveModel = require('../../models/seo/SeoTestDriveModel');

router.post('/testdrive-seo', async (req, res) => {
  try {
    const existingData = await SeoTestDriveModel.findOne();

    if (!existingData) {
      const createData = new SeoTestDriveModel({
        meta_title: {
...
router.get('/testdrive-seo', async (req, res) => {
  try {
    const data = await SeoTestDriveModel.find();
...
router.get('/testdrive-seo-front', async (req, res) => {
  try {
    const acceptLanguage = req.headers['accept-language'];
    const preferredLanguage = acceptLanguage.split(',')[0].split(';')[0];

    const datas = await SeoTestDriveModel.find();
