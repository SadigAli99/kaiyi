...
const RepairSeoModel = require('../../models/seo/RepairSeoModel');

router.post('/repair-seo', async (req, res) => {
  try {
    const existingData = await RepairSeoModel.findOne();

    if (!existingData) {
      const createData = new RepairSeoModel({
        meta_title: {
...
router.get('/repair-seo', async (req, res) => {
  try {
    const data = await RepairSeoModel.find();
...
router.get('/repair-seo-front', async (req, res) => {
  try {
    const acceptLanguage = req.headers['accept-language'];
    const preferredLanguage = acceptLanguage.split(',')[0].split(';')[0];

    const datas = await RepairSeoModel.find();
