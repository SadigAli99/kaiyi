...
const SeoCorporateModel = require('../../models/seo/SeoCorporateModel');

router.post('/forcorporate-seo', async (req, res) => {
  try {
    const existingData = await SeoCorporateModel.findOne();

    if (!existingData) {
      const createData = new SeoCorporateModel({
        meta_title: {
...
router.get('/forcorporate-seo', async (req, res) => {
  try {
    const data = await SeoCorporateModel.find();
...
router.get('/forcorporate-seo-front', async (req, res) => {
  try {
    const acceptLanguage = req.headers['accept-language'];
    const preferredLanguage = acceptLanguage.split(',')[0].split(';')[0];

    const datas = await SeoCorporateModel.find();
