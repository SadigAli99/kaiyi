...
const KaiyiGarantSeoModel = require('../../models/seo/KaiyiGarantSeoModel');

router.post('/kaiyigarant-seo', async (req, res) => {
  try {
    const existingData = await KaiyiGarantSeoModel.findOne();

    if (!existingData) {
      const createData = new KaiyiGarantSeoModel({
        meta_title: {
...
router.get('/kaiyigarant-seo', async (req, res) => {
  try {
    const data = await KaiyiGarantSeoModel.find();
...
router.get('/kaiyigarant-seo-front', async (req, res) => {
  try {
    const acceptLanguage = req.headers['accept-language'];
    const preferredLanguage = acceptLanguage.split(',')[0].split(';')[0];

    const datas = await KaiyiGarantSeoModel.find();
