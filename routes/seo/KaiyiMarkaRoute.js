...
const KaiyiMarkaModel = require('../../models/seo/KaiyiMarkaModel');

router.post('/kaiyimarka-seo', async (req, res) => {
  try {
    const existingData = await KaiyiMarkaModel.findOne();

    if (!existingData) {
      const createData = new KaiyiMarkaModel({
        meta_title: {
...
router.get('/kaiyimarka-seo', async (req, res) => {
  try {
    const data = await KaiyiMarkaModel.find();
...
router.get('/kaiyimarka-seo-front', async (req, res) => {
  try {
    const acceptLanguage = req.headers['accept-language'];
    const preferredLanguage = acceptLanguage.split(',')[0].split(';')[0];

    const datas = await KaiyiMarkaModel.find();
