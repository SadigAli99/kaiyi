...
const RoadRulesModel = require('../../models/seo/RoadRulesModel');

router.post('/roadrules-seo', async (req, res) => {
  try {
    const existingData = await RoadRulesModel.findOne();

    if (!existingData) {
      const createData = new RoadRulesModel({
        meta_title: {
...
router.get('/roadrules-seo', async (req, res) => {
  try {
    const data = await RoadRulesModel.find();
...
router.get('/roadrules-seo-front', async (req, res) => {
  try {
    const acceptLanguage = req.headers['accept-language'];
    const preferredLanguage = acceptLanguage.split(',')[0].split(';')[0];

    const datas = await RoadRulesModel.find();
