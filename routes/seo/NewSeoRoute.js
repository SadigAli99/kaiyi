...
const NewSeoModel = require('../../models/seo/NewSeoModel');

router.post('/new-seo', async (req, res) => {
  try {
    const existingData = await NewSeoModel.findOne();
    if (!existingData) {
      const createData = new NewSeoModel({
        meta_title: {
...
router.get('/new-seo', async (req, res) => {
  try {
    const data = await NewSeoModel.find();
...
router.get('/new-seo-front', async (req, res) => {
  try {
    const acceptLanguage = req.headers['accept-language'];
    const preferredLanguage = acceptLanguage.split(',')[0].split(';')[0];

    const datas = await NewSeoModel.find();
