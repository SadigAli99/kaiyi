...
const PointSaleModel = require('../../models/seo/PointSaleModel');

router.post('/pointsale-seo', async (req, res) => {
  try {
    const existingData = await PointSaleModel.findOne();

    if (!existingData) {
      const createData = new PointSaleModel({
        meta_title: {
...
router.get('/pointsale-seo', async (req, res) => {
  try {
    const data = await PointSaleModel.find();
...
router.get('/pointsale-seo-front', async (req, res) => {
  try {
    const acceptLanguage = req.headers['accept-language'];
    const preferredLanguage = acceptLanguage.split(',')[0].split(';')[0];

    const datas = await PointSaleModel.find();
