...
const SeoBlogModel = require('../../models/seo/SeoBlogModel');
...
router.get('/blog-seo', async (req, res) => {
  try {
    const data = await SeoBlogModel.find();
...
router.get('/blog-seo-front', async (req, res) => {
  try {
    const acceptLanguage = req.headers['accept-language'];
    const preferredLanguage = acceptLanguage.split(',')[0].split(';')[0];

    const datas = await SeoBlogModel.find();
