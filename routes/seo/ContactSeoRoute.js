...
const ContactSeoModel = require('../../models/seo/ContactSeoModel');

router.post('/contact-seo', async (req, res) => {
  try {
    const existingData = await ContactSeoModel.findOne();
    if (!existingData) {
      const createData = new ContactSeoModel({
        meta_title: {
...
router.get('/contact-seo', async (req, res) => {
  try {
    const data = await ContactSeoModel.find();
...
router.get('/contact-seo-front', async (req, res) => {
  try {
    const acceptLanguage = req.headers['accept-language'];
    const preferredLanguage = acceptLanguage.split(',')[0].split(';')[0];

    const datas = await ContactSeoModel.find();
